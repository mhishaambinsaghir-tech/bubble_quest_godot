...
func world_to_grid(world_position: Vector2) -> Vector2i:
	...
	var row = int(
		round(local_position.y / (bubble_spacing * 0.866))
	)
	...
	var col = int(
		round((local_position.x - x_offset) / bubble_spacing)
	)
...
func attach_bubble(bubble: Area2D, hit_bubble: Area2D) -> void:
	...
	var hit_cell = world_to_grid(hit_bubble.global_position)
	...
	for neighbor in neighbors:
		...
		if not grid.has(neighbor):
			...
			var distance = bubble.global_position.distance_to(
				world_position
			)
...
func attach_bubble_to_ceiling(bubble: Area2D) -> void:
	...
	print("Bubble attached to ceiling cell: ", best_cell)
...
func finish_bubble_turn(cell: Vector2i) -> void:
	...
	print("Matching bubbles found: ", matches.size())
	...
	if matches.size() >= 3:
		...
		var floating_count = remove_floating_bubbles()

		var match_score = matches.size() * 10
		var floating_score = floating_count * 20

		var total_score = match_score + floating_score

		game_manager.add_score(total_score)

		print("Matched bubbles: ", matches.size())
		print("Floating bubbles: ", floating_count)
		print("Score gained: ", total_score)
...
func find_matching_bubbles(start_cell: Vector2i) -> Array:
	...
	var matches = []
	var to_check = [start_cell]
	var checked = {}

	var start_bubble = grid.get(start_cell)

	if start_bubble == null:
		return matches

	var target_type = start_bubble.bubble_type

	while to_check.size() > 0:

		var current_cell = to_check.pop_back()
		...
		var current_bubble = grid[current_cell]

		if current_bubble.bubble_type != target_type:
			continue

		matches.append(current_bubble)
...
func remove_floating_bubbles() -> int:
	...
	var connected = find_ceiling_connected()
